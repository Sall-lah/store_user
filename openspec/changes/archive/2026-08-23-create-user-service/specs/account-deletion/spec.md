...
- **WHEN** an authenticated user with active in-flight orders requests DELETE /api/v1/users/account
...
- **WHEN** an authenticated user requests DELETE /api/v1/users/account and store_order gRPC service is unavailable or times out
...
- **WHEN** an authenticated user with zero active orders requests DELETE /api/v1/users/account
